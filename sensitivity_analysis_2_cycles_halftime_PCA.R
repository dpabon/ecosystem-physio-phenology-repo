...
for (st in 1:nrow(m5)) {
  if (m5$FILTER2[st] == 1) {
    ...
    # GPP
    load(paste("data/fluxnet/FLUXNET_sites/GPPsat_talie/", m5$SITE_ID[st], ".Rdata",
               sep = ""))
...
site.1.2 <- vector()
length.days <- vector()
length.days.1.1 <- vector()
length.days.1.2 <- vector()
...
for (deus in 1:length(deus.ex)) {
  ...
  # two growing seassons filter
  m5$FILTER2[which(m5$n.cycles == 1)] <- 0
  ...
  for (st in 1:nrow(m5)) {
    if (m5$FILTER2[st] == 1) {
      if (m5$FILTER2[st] == 1 & m5$n.cycles[st] == 1) {
        ...
        #   climatic[, 5] <- scal(gpp.ef[st][[1]][[2]])
        #   if (any(is.na(climatic))) {
        #     timing <- sites.info[st][[1]][[2]][-which(is.na(climatic), arr.ind = T)[,
        #                                                                             1]]
        ...
        #                                            init = c(0, 0, 0, 0), type = "c-l", verbose = F), silent = T), timeout = 10,
        #                            onTimeout = "warning")
        # 
        #     if ((class(circres) == "try-error") || is.null(circres)) {
        #       temporal.bot[bot.i, 1] <- NA
        #       temporal.bot[bot.i, 2:6] <- NA
        #     } else {
        #       temporal.bot[bot.i, 1] <- circres$mu
        #       temporal.bot[bot.i, 2:6] <- circres$coef
        #     }
        #   }
        #   trew[st][[1]][[1]] <- temporal.bot  # raw data
        #   trew[st][[1]][[2]] <- apply(temporal.bot[, 2:6], MARGIN = 2, mean,
        #                               na.rm = T)  # averages (coefficients)
        #   trew[st][[1]][[3]] <- apply(temporal.bot[, 2:6], MARGIN = 2, FUN = sd,
        #                               na.rm = T)  # sd  coefficients)
        #   trew[st][[1]][[4]] <- mean.circular(circular(temporal.bot[, 1],
        #                                                units = "radians", modulo = "2pi", rotation = "clock", zero = pi/2),
        #                                       na.rm = T)  # mu mean
        #   #trew[st][[1]][[5]] <- sd.circular(circular(temporal.bot[, 1], units = "radians",
        #   #                                           modulo = "2pi", rotation = "clock", zero = pi/2), na.rm = T)  # mu sd
        ...
        if (is.null(sites.info[[st]]) == FALSE) {
          for (d in 1:2) {
            ...
            if (any(is.na(climatic))) {
              timing <- timing[-which(is.na(climatic), arr.ind = T)[,1]]
              climatic <- na.omit(climatic)
            }
            ...
            new.climatic <- matrix(NA, nrow = nrow(climatic),
                                   ncol = 2)
            ...
            if (d == 1) {
              trew.1.1[st][[1]][[1]] <- temporal.bot # raw data 
              trew.1.1[st][[1]][[2]] <-  apply(temporal.bot[,2:3], MARGIN = 2, mean, na.rm = T) # averages (coefficients)
              trew.1.1[st][[1]][[3]] <- apply(temporal.bot[,2:3], MARGIN = 2, FUN = sd, na.rm = T) # sd  coefficients)
              trew.1.1[st][[1]][[4]] <- mean.circular(circular(temporal.bot[,1],units = "radians", modulo = "2pi", rotation = "counter", zero = 0), na.rm = T)  # mu mean
              
              #trew.1.1[st][[1]][[5]] <- sd.circular(circular(temporal.bot[,1],units = "radians", modulo = "2pi", rotation = "clock", zero = pi/2), na.rm = T) # mu sd 
              #trew.1.1[st][[1]][[6]] <-  mean(temporal.bot[,6], na.rm = T) # kappa average
              #trew.1.1[st][[1]][[7]] <- sd(temporal.bot[,6], na.rm = T) # kappa sd
              #trew.1.1[st][[1]][[8]] <- sd(timing, na.rm = T) # deviation of the number of days (linear)
            }else{
              trew.1.2[st][[1]][[1]] <- temporal.bot # raw data 
              trew.1.2[st][[1]][[2]] <-  apply(temporal.bot[,2:3], MARGIN = 2, mean, na.rm = T) # averages (coefficients)
              trew.1.2[st][[1]][[3]] <- apply(temporal.bot[,2:3], MARGIN = 2, FUN = sd, na.rm = T) # sd  coefficients)
              trew.1.2[st][[1]][[4]] <- mean.circular(circular(temporal.bot[,1],units = "radians", modulo = "2pi", rotation = "counter", zero = 0), na.rm = T)  # mu mean
              
              #trew.1.2[st][[1]][[5]] <- sd.circular(circular(temporal.bot[,1],units = "radians", modulo = "2pi", rotation = "clock", zero = pi/2), na.rm = T) # mu sd 
              #trew.1.2[st][[1]][[6]] <-  mean(temporal.bot[,6], na.rm = T) # kappa average
              #trew.1.2[st][[1]][[7]] <- sd(temporal.bot[,6], na.rm = T) # kappa sd
              #trew.1.2[st][[1]][[8]] <- sd(timing, na.rm = T) # deviation of the number of days (linear)
            }
          }
        }
      }
    ...
    for (st in 1:nrow(m5)) {
      if (m5$FILTER2[st] == 1 & m5$n.cycles[st] == 2) {
        if (is.null(unlist(trew.1.1[st][[1]][[2]])) == F | is.null(unlist(trew.1.2[st][[1]][[2]])) == F) {
          for (d in 1:2) {
            if (all(is.nan(trew.1.1[st][[1]][[2]])) == FALSE | all(is.nan(trew.1.2[st][[1]][[2]]))  == FALSE) {
              timing <- sites.info[st][[1]][[2]][which(sites.info[st][[1]][[5]] == d)]
              timing.circ <- circular(timing * (360/365) * (pi/180), units = "radians",
                                      modulo = "2pi", rotation = "counter", zero = 0)
              climatic <- matrix(NA, nrow = length(sites.info[st][[1]][[1]][which(sites.info[st][[1]][[5]] == d)]),
                                 ncol = 4)
              climatic[, 1] <- scal(gpp.tair[st][[1]][[2]][which(sites.info[st][[1]][[5]] == d)])
              climatic[, 2] <- scal(gpp.swrad[st][[1]][[2]][which(sites.info[st][[1]][[5]] == d)])
              climatic[, 3] <- scal(gpp.precip[st][[1]][[2]][which(sites.info[st][[1]][[5]] == d)])
              ...
              if (d == 1) {
                sim <- vector()
                for (i in 1:nrow(new.climatic)) {
                  x <- new.climatic[i, ]
                  sim <- c(sim, as.numeric(circ.recons(x, mu = as.numeric(trew.1.1[st][[1]][[4]]),
                                                       coef = trew.1.1[st][[1]][[2]])))
                }
                # sim <- sim / (360/365) / (pi/180)
                test <- circular(sim, units = "radians", modulo = "2pi", rotation = "counter",
                                 zero = 0)
                ex <- cor.circular(timing.circ, test, test = T)$cor
                # plot(as.numeric(timing.circ)) points(as.numeric(sim), col = 'red')
                rsqrt.1.1 <- c(rsqrt.1.1, cor.circular(timing.circ, test, test = T)$cor)
                name.1.1 <- c(name.1.1, st)
              }else {
                sim <- vector()
                for (i in 1:nrow(new.climatic)) {
                  x <- new.climatic[i, ]
                  sim <- c(sim, as.numeric(circ.recons(x, mu = as.numeric(trew.1.2[st][[1]][[4]]),
                                                       coef = trew.1.2[st][[1]][[2]])))
                }
                # sim <- sim / (360/365) / (pi/180)
                test <- circular(sim, units = "radians", modulo = "2pi", rotation = "counter",
                                 zero = 0)
                ex <- cor.circular(timing.circ, test, test = T)$cor
                # plot(as.numeric(timing.circ)) points(as.numeric(sim), col = 'red')
                rsqrt.1.2 <- c(rsqrt.1.2, cor.circular(timing.circ, test, test = T)$cor)
                name.1.2 <- c(name.1.2, st)
              }
              
            } else {
              rsqrt.1.1 <- c(rsqrt.1.1, NA)
              name.1.1 <- c(name.1.1, st)
              rsqrt.1.2 <- c(rsqrt.1.2, NA)
              name.1.2 <- c(name.1.2, st)
  ...
  length.days.1.1 <- c(length.days.1.1, rep(deus.ex[deus], length(rsqrt.1.1)))
  length.days.1.2 <- c(length.days.1.2, rep(deus.ex[deus], length(rsqrt.1.2)))
  site.1.1 <- c(site.1.1, name.1.1)
  site.1.2 <- c(site.1.2, name.1.2)
  rsqrt.2.1.1 <- c(rsqrt.2.1.1, rsqrt.1.1)
  rsqrt.2.1.2 <- c(rsqrt.2.1.2, rsqrt.1.2)
...
#                               jama = rsqrt.2)
test.data.frame.1.1 <- data.frame(sites = m5$SITE_ID[site.1.1], half.time = length.days.1.1,
                              jama = rsqrt.2.1.1)
test.data.frame.1.2 <- data.frame(sites = m5$SITE_ID[site.1.2], half.time = length.days.1.2,
                              jama = rsqrt.2.1.2)
# write.csv(test.data.frame, paste("results/sensitivity_half_time_", c.job, ".csv", sep = ""))

write.csv(test.data.frame.1.1, paste("results/sensitivity_half_time_1_", c.job, ".csv", sep = ""))
write.csv(test.data.frame.1.2, paste("results/sensitivity_half_time_2_", c.job, ".csv", sep = ""))
